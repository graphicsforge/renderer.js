...
<script id="fs-raymarch" type="fragment">
  #ifdef GL_ES
  precision highp float;
  #endif
  uniform vec3 iResolution;
  //uniform float iGlobalTime;
  //uniform vec3 iChannelTime[4];
  //uniform vec4 iMouse;
  ...
  float distToSphere( in vec3 p, in float radSq )
  {
    return max(dot(p, p)-radSq, 0.);
  }
  ...
  vec2 column( in float x, in float y, in float z )
  {
    ...
    float di3=distToSphere( p+vec3(-.1,.0,.1), .1 );

      float dm = max(di3, di2);
    
    vec2 res = vec2( dm, 1.0 );
  ...
  vec3 calcColor( in vec3 pos, in vec3 nor, in float sid )
  {
      ...
      if( sid<0.5 )
    {
      ...
      float de = 1.;//20.0*length(fwidth(pos.xz));
  ...
  { 
      ...
      for( int i=0; i<7; i++ )
    {
      ...
      vec3 col = vec3(0.0);
      vec3 res = castRay(ro,rd,0.001,0.015,20.0);
      float t = res.x;
      if( res.y>-0.5 )
      {
          vec3 pos = ro + t*rd;
...
//      vec2 mo = iMouse.xy/iResolution.xy;
    ...
    // camera 
    vec3 ce = vec3( 1.5, 1., 0.5 );
    vec3 ro = ce;// + vec3( 1.3*cos(0.11*time + 6.0*mo.x), 0.65*(1.0-mo.y), 1.3*sin(0.11*time + 6.0*mo.x) );
    vec3 ta = vec3(0.,0.,0.);// + vec3( 0.95*cos(1.2+.08*time), 0.4*0.25+0.75*ro.y, 0.95*sin(2.0+0.07*time) );
